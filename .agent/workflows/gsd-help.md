...
**`/gsd:prepare-testing`**
Analyze codebase and generate testing workflows.

- Scans project for pages, components, APIs
- Reviews `.planning/` for user journeys
- Creates `.testing/` folder with test workflow files
- Sets up `.backlog/simple/` and `.backlog/complex/` folders

Usage: `/gsd:prepare-testing`
Usage: `/gsd:prepare-testing "auth feature"`

**`/gsd:start-testing <file>`**
Execute a testing workflow using browser automation.

- Loads test workflow from `.testing/`
- Runs each test case via browser
- Captures screenshots and recordings
- Logs failures to `.backlog/simple/` or `.backlog/complex/`

Usage: `/gsd:start-testing auth-testing.md`

**`/gsd:start-debugging <simple|complex>`**
Work through bugs in a backlog folder.

- Reads and prioritizes bugs in backlog
- For each: analyze, fix, verify, commit
- Escalates bugs if scope increases
- Archives fixed bugs

Usage: `/gsd:start-debugging simple`
Usage: `/gsd:start-debugging complex`

### Context Management

**`/gsd:compact`**
Compact context to prevent context window rot.

- Summarizes session context into STATE.md
- Preserves critical decisions and issues
- Creates checkpoint for resumption
- Use after long sessions or before complex work
